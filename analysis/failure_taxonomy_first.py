...
def truncate_and_concatenate(claim_text, document_texts, tokenizer, max_total_length=512):
    ...
    #fixing to avoid error as token indices sequence length is longer and adding truncation
    claim_tokens = tokenizer.encode(claim_text, add_special_tokens=False, truncation=True, max_length=max_total_length)
    ...
    for document_text in document_texts:
        ...
        #fixing to avoid error as token indices sequence length is longer and adding truncation
        document_tokens = tokenizer.encode(document_text, add_special_tokens=False, truncation=True, max_length=max_total_length)
