...
whereas this project uses a fully fine-tuned RoBERTa classifier reading claim + evidence. The two are therefore not directly comparable in absolute performance, and no such comparison is drawn; the connection is limited to retrieval depth and the context-length observation.
